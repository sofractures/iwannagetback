...
class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.bossTrailParticles = this.add.particles(0, 0, 'ethereal', {
      ...
      quantity: 3, // Fewer particles to avoid spam
      emitting: false,
      frequency: 200, // Less frequent emission
    ...
    this.bossSpawnedAt.forEach((milestone, index) => {
      ...
      if (this.score >= milestone && !this.bossesSpawned[index] && !this.currentBoss && enoughGapSinceLastBoss) {
        ...
        try {
          ...
          console.error(`Error spawning boss ${index + 1}:`, error)
        }
      }
    })
  ...
  private triggerAutoEscape() {
    ...
    if (playerBody.touching.down) {
      playerBody.setVelocityY(-320) // Moderate jump
  ...
  private handleBossCollision(boss: Phaser.GameObjects.Sprite) {
    // Check if player is attacking (space key or up arrow)
    const isAttacking = this.cursors.space?.isDown || this.cursors.up?.isDown
    ...
    if (isAttacking) {
      ...
      // Player takes damage when touching boss without attacking
      this.lives--
      this.livesText.setText(`Lives: ${this.lives}`)
  ...
  private updateLevelSpeed(delta: number) {
    // Increase speed over time for each level
    if (this.gameStarted && !this.currentBoss) {
      this.levelStartTime += delta
      
      // Increase speed gradually over time
      const speedIncrease = Math.min(this.levelStartTime * this.levelSpeedIncrease / 1000, this.maxLevelSpeed - 1.0)
      this.levelSpeed = 1.0 + speedIncrease
      
      // Update level display
      const newLevel = Math.floor(speedIncrease / 0.5) + 1
  ...
  private spawnBoss(bossNumber: number) {
    ...
    // Set boss health based on boss number
    this.maxBossHealth = bossNumber // Boss 1: 1 hit, Boss 2: 2 hits, Boss 3: 3 hits
    ...
    try {
      this.currentBoss = this.bosses.create(this.scale.width + 100, this.scale.height / 2, bossKey) as any
    } catch (error) {
      console.error(`Failed to create boss sprite with key: ${bossKey}`, error)
      return
    }
    (this.currentBoss as any).setScale(1.5)
    (this.currentBoss as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityX(-100) // Move left slowly
    
    // Set boss physics
    (this.currentBoss.body as Phaser.Physics.Arcade.Body).setSize(this.currentBoss.width * 0.8, this.currentBoss.height * 0.8)
    (this.currentBoss.body as Phaser.Physics.Arcade.Body).setImmovable(true)
    ...
    try {
      this.bossSpawnSound = this.sound.add('bossSpawnSound', { volume: this.isMuted ? 0 : 0.6 })
    ...
    // Start trail particles for Boss 1 and 2
  ...
  private createBossHealthBar() {
    ...
    const healthBarBg = this.add.rectangle(this.scale.width / 2, 50, 200, 20, 0x000000, 0.8)
    healthBarBg.setScrollFactor(0)
    healthBarBg.setDepth(1000)
    
    // Create health bar fill
    const healthBarFill = this.add.rectangle(this.scale.width / 2, 50, 200, 20, 0xff0000, 1)
    healthBarFill.setScrollFactor(0)
    healthBarFill.setDepth(1001)
    
    // Store references for updating
    this.bossHealthBarBg = healthBarBg
    this.bossHealthBarFill = healthBarFill
  }

  private updateBossHealthBar() {
    if (!this.bossHealthBarFill || !this.bossHealthBarBg) return
    
    const healthPercentage = this.bossHealth / this.maxBossHealth
    const barWidth = 200 * healthPercentage
    
    // Update health bar fill width
    this.bossHealthBarFill.setDisplaySize(barWidth, 20)
    
    // Change color based on health
    if (healthPercentage > 0.6) {
      this.bossHealthBarFill.setFillStyle(0x00ff00) // Green
    } else if (healthPercentage > 0.3) {
      this.bossHealthBarFill.setFillStyle(0xffff00) // Yellow
    } else {
      this.bossHealthBarFill.setFillStyle(0xff0000) // Red
    }
  }
  ...
  private updateBossMovement(delta: number) {
    ...
    this.currentBoss.y = this.scale.height / 2 + floatAmount
    ...
    if (this.currentBoss.x < rightSideMin) {
      (this.currentBoss as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityX(50) // Move right
      this.currentBoss.setTexture(this.currentBoss.texture.key.replace('_left', '_right'))
    } else if (this.currentBoss.x > rightSideMax) {
      (this.currentBoss as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityX(-50) // Move left
      this.currentBoss.setTexture(this.currentBoss.texture.key.replace('_right', '_left'))
    } else {
      // Boss is in the right zone - slow movement
      (this.currentBoss as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityX(0)
    }
  }
  ...
  private updateBossAttacks(delta: number) {
    ...
    if (this.bossAttackTimer >= this.bossAttackInterval) {
      ...
      if (bossNumber === 1) {
        ...
        const laser = this.bossLasers.create(this.currentBoss.x, this.currentBoss.y, 'boss1_laser') as any
        laser.setVelocityX(-this.laserSpeed) // Always negative (left direction)
        (laser.body as Phaser.Physics.Arcade.Body).setSize(laser.width * 0.5, laser.height * 0.5)
        ...
        // Boss 2: Rotating hat attack
        const hat = this.bossLasers.create(this.currentBoss.x, this.currentBoss.y, 'boss2_hat') as any
        hat.setVelocityX(-this.laserSpeed * 0.8) // Slightly slower
        (hat.body as Phaser.Physics.Arcade.Body).setSize(hat.width * 0.5, hat.height * 0.5)
  ...
  private spawnDrone() {
    ...
    // Create drone
    const drone = this.bossDrones.create(this.currentBoss.x, this.currentBoss.y + 50, 'drone') as any
    (drone as any).setScale(0.8)
    (drone.body as Phaser.Physics.Arcade.Body).setSize(drone.width * 0.8, drone.height * 0.8)
    
    // Set drone data
    (drone as any).setData('hoverTimer', 0)
    (drone as any).setData('bombTimer', 0)
    (drone as any).setData('isHovering', false)
  }
  ...
  private updateDroneHovering(droneSprite: Phaser.GameObjects.Sprite, delta: number) {
    ...
    (droneSprite as any).setData('hoverTimer', hoverTimer)
    (droneSprite as any).setData('bombTimer', bombTimer)
    
    if (!isHovering) {
      // Start hovering
      (droneSprite as any).setData('isHovering', true)
      (droneSprite as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityX(0)
      (droneSprite as Phaser.Types.Physics.Arcade.SpriteWithDynamicBody).setVelocityY(0)
    }
    
    // Drop bombs every 2 seconds while hovering
    if (bombTimer >= 2000) {
      (droneSprite as any).setData('bombTimer', 0)
      
      // Create bomb
      const bomb = this.droneBombs.create(droneSprite.x, droneSprite.y, 'bomb') as any
      (bomb as any).setScale(0.6)
      (bomb.body as Phaser.Physics.Arcade.Body).setSize(bomb.width * 0.8, bomb.height * 0.8)
