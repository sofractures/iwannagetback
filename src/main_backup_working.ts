...
import { submitScore, testSupabaseConnection } from './supabase'
...
class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    testSupabaseConnection().then(success => {
      if (success) {
        console.log('✅ Supabase leaderboard connection is working!')
      } else {
  ...
  update(time: number, delta: number) {
    ...
    if (!this.currentBoss) {
      ...
      if (gapSinceLastBoss >= 1000) {
        ...
        if (bossToSpawn > 0) {
          console.log(`✅ Spawning Boss ${bossToSpawn} at wellbeing ${this.score}`)
  ...
  private showWinnerScreen() {
    ...
    // Show subtext
    this.winnerSubText = this.add.text(this.scale.width / 2, this.scale.height / 2 + 20, 'You have achieved a health life balance', {
      fontFamily: 'ui, Arial, sans-serif',
      fontSize: '24px',
      color: '#ffffff',
      stroke: '#000000',
    ...
    // Play game win sound
    try {
      this.gameWinSound = this.sound.add('gameWinSound', { volume: this.isMuted ? 0 : 0.6 })
    ...
    // Start 3-second timer before showing leaderboard
    this.leaderboardTimer = this.time.addEvent({
  ...
  private updateDroneHovering(droneSprite: Phaser.GameObjects.Sprite, delta: number) {
    ...
    if (droneSprite.x >= screenWidth - droneWidth / 2) {
      // Hit right edge - reverse to left
      droneSprite.setData('direction', -1)
    } else if (droneSprite.x <= droneWidth / 2) {
