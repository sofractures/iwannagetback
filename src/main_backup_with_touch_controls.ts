...
import { submitScore, testSupabaseConnection, getLeaderboard } from './supabase'
...
class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    testSupabaseConnection().then(success => {
      if (success) {
        ...
        console.log('❌ Supabase leaderboard connection failed!')
      }
    })
  ...
  private resetAllGameState() {
    ...
    this.bossLaserInterval = 3000
    this.bossHatTimer = 0
    this.bossHatInterval = 2500
    this.bossDroneTimer = 0
    this.bossDroneInterval = 4000
  ...
  private setupLeaderboardInput() {
    ...
    this.input.keyboard!.on('keydown', (event: KeyboardEvent) => {
      ...
      if (!this.initialsInput || !this.emailInput || this.currentInput === 'none') {
  ...
  private async submitLeaderboardScore() {
    ...
    try {
      ...
      const success = await submitScore(this.initialsText, this.emailText, this.score, this.playerWon)
  ...
  private async showTop10Leaderboard() {
    ...
    if (this.submitErrorText) {
      this.submitErrorText.setVisible(false)
    }
    if (this.submitSuccessText) {
      this.submitSuccessText.setVisible(false)
    }
    ...
    try {
      ...
      topScores.forEach((entry, index) => {
        ...
        // Winner indicator
        if (isWinner) {
          const winnerText = this.add.text(scoreX + scoreWidth + 6, yOffset, '🏆', {
            fontFamily: 'ui, Arial, sans-serif',
            fontSize: '14px', // Further reduced from 16px
          })
          winnerText.setOrigin(0, 0.5)
          winnerText.setDepth(10002)
          this.leaderboardScreen.add(winnerText)
        }
      })
    ...
    continueButton.on('pointerdown', () => this.restartGame())
  ...
  private cleanupLeaderboardScreen() {
    ...
    if (this.submitErrorText) {
      this.submitErrorText.destroy()
      this.submitErrorText = undefined
      console.log('🧹 Destroyed error text')
    }
    if (this.submitSuccessText) {
      this.submitSuccessText.destroy()
      this.submitSuccessText = undefined
      console.log('🧹 Destroyed success text')
    }
  ...
  private createTouchControls() {
    ...
    // Left button (circle)
    this.leftButton = this.add.circle(leftX, yPosition, buttonSize/2, 0x333333, buttonAlpha)
      .setInteractive()
    ...
    // Right button (circle)
    this.rightButton = this.add.circle(rightX, yPosition, buttonSize/2, 0x333333, buttonAlpha)
      .setInteractive()
    ...
    // Jump button (circle)
    this.jumpButton = this.add.circle(jumpX, yPosition, buttonSize/2, 0x4CAF50, buttonAlpha)
      .setInteractive()
